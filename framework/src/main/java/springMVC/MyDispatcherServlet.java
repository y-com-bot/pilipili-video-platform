...
public class MyDispatcherServlet extends HttpServlet {
    ...
    private void startLifecycleBeans() {
        LinkedHashSet<Object> uniqueBeans = new LinkedHashSet<>(iocContainer.values());
        for (Object bean : uniqueBeans) {
            if (bean instanceof MyLifecycle lifecycle) {
                lifecycle.start();
            }
        }
    }
    ...
    private void doDispatch(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        String url = normalizeUrl(requestURI.replace(contextPath, ""));
        
        System.out.println("[DEBUG] requestURI: " + requestURI);
        System.out.println("[DEBUG] contextPath: " + contextPath);
        System.out.println("[DEBUG] normalized url: " + url);
        System.out.println("[DEBUG] handlerMapping keys: " + handlerMapping.keySet());
        
        Method method = handlerMapping.get(url);
        Object controller = urlControllerMap.get(url);

        if (method == null || controller == null) {
            System.out.println("[DEBUG] No handler found, trying static resource...");
    ...
    private boolean tryForwardStaticResource(String url,
        ...
        // 处理根路径，加载欢迎文件
        if ("/".equals(url)) {
            System.out.println("[DEBUG] Root path detected, redirecting to /index.html");
            url = "/index.html";
        }

        System.out.println("[DEBUG] Looking for static resource: " + url);
        if (req.getServletContext().getResource(url) == null) {
            System.out.println("[DEBUG] Resource not found: " + url);
            return false;
        }

        System.out.println("[DEBUG] Found static resource, serving: " + url);
        ...
        try (InputStream inputStream = req.getServletContext().getResourceAsStream(url);
            ...
            if (inputStream == null) {
                System.out.println("[DEBUG] InputStream is null for: " + url);
                return false;
            }
            ...
            outputStream.flush();
            System.out.println("[DEBUG] Successfully served: " + url);
            return true;
        }
    }
